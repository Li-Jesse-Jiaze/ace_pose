...
class CamLocDataset(Dataset):
    ...
    def __init__(self,
        ...
        if ace_pose_file is not None:
            ...
            if len(self.rgb_files) > max_images:
                total_images = len(self.rgb_files)
                step = total_images / max_images
                selected_indices = [int(math.floor(step * i)) for i in range(max_images)]
                ...
                if self.poses:
                    self.poses = [self.poses[i] for i in selected_indices]
        ...
        if pose_seed > -1:
            ...
            if len(self.depth_files) > 0:
                ...
                # estimate depth
                _logger.info("Using ZoeDepth for depth initialization.")
