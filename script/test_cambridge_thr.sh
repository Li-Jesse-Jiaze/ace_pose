...
datasets=("KingsCollege" "OldHospital" "ShopFacade" "StMarysChurch" "GreatCourt")
...
for dataset in "${datasets[@]}"; do
    ...
    for threshold in "${patch_thresholds[@]}"; do
        ...
        results_folder="results_Cambridge/${dataset}_threshold_${threshold}"
        mkdir -p "$results_folder"
        
        # Define paths
        train_rgb_path="data/Cambridge_${dataset}/train/rgb/*.png"
        train_pose_path="data/Cambridge_${dataset}/train/poses/*.txt"
        test_rgb_path="data/Cambridge_${dataset}/test/rgb/*.png"
        test_pose_path="data/Cambridge_${dataset}/test/poses/*.txt"
        ...
        python eval_poses.py "$results_folder/poses_query.txt" "$test_pose_path" --pose_error_thresh_t 1 2>&1 | tee "$eval_output"
